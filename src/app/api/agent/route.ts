...
表示法：preset <cartoon|ballstick|spacefill|wireframe|surface|bindingsite|publication|hybrid|putty>（publication=出版级互作一键组合：蛋白 cartoon 链色 + 配体碳鲜绿 + 口袋残基碳按到配体距离紫→粉渐变（N 蓝 O 红 S 黄杂原子元素色）+ 智能主链（仅氢键参与者的主链 O/N 显示棍棒），同时清除旧烘焙色） · show <rep> <sel>（rep 与 sel 用空格或逗号分隔均可：show ballstick, ligand ≡ show ballstick ligand；rep: cartoon/putty/ballstick/sticks/lines/spacefill/surface） · hide <rep|all> <sel> · show hydrogens / hide hydrogens / show waters / hide waters
...
着色：color <方案|颜色> <sel>（逗号/空格分隔均可；方案: element/pocket/chain/spectrum/residue/ss/bfactor/sasa/uniform，pocket=口袋专业配色：配体碳鲜绿+残基碳按到配体距离紫→粉渐变+杂原子元素色；或 red/#ff8800） · util cbc|cnc|ss|cbaw · reset_colors · bg <颜色>
...
视觉：set <项> <值>（项: ambient direct fill specular fog fog_strength fov spin_speed transition quick|normal|cinematic quality low|medium|high axes fps seq_focus cap_color cap_shading auto_perf outline outline_strength outline_thickness transparency sphere_scale stick_radius cartoon_width） · spin on|off · rock on|off · slab <nÅ>|off|move <±Å>|center|cap on|off · stereo on|off · ssao on|off [半径Å] [强度]（独立命令，非 set 键） · outline on|off [强度 粗细px] · axes on|off · fps on|off · label on|off（标记当前选择） · hbonds on [nÅ]（默认仅选择集范围；无选择时不显示） · hbonds on [nÅ] in <表达式>（烘焙独立范围：不随 deselect 清除、不依赖选择——口袋工作流标准写法 hbonds on 3.4 in byres(within 4.5 of (ligand)) and not water，范围必须含配体本身才能画出配体-残基氢键） · hbonds off · symmetry <Å>|off · map fofc <id>（差值电子密度）
...
18. 出版级图标准流程（「出版级/投稿图/高清图/互作图/药物-蛋白结合图/分析结合位点出图/展示口袋」类需求，按此顺序，ray 必须是最后一条）：
   ...
   ② preset publication（cartoon 链色 + 配体碳鲜绿 + 口袋残基按到配体距离紫→粉渐变 + 杂原子元素色 + 智能主链——氢键虚线出现时参与互作的主链 O/N 自动补显）
